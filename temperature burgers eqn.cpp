...
#define TI_IMPLICIT 0
#define TI_EXPLICIT 1

int TIME_SCHEME = TI_IMPLICIT;

const char* TIME_SCHEME_NAME()
{
    if (TIME_SCHEME == TI_EXPLICIT) return "EXPLICIT (forward Euler)";
    else return "IMPLICIT (backward Euler, GS)";
}

int PARSE_TIME_SCHEME(const char* s)
{
    if(!s) return -1;
    if(!strcmp(s,"0") || !strcmp(s,"imp") || !strcmp(s,"IMP") || !strcmp(s,"implicit")) return TI_IMPLICIT;
    if(!strcmp(s,"1") || !strcmp(s,"exp") || !strcmp(s,"EXP") || !strcmp(s,"explicit")) return TI_EXPLICIT;
    return -1;
}
...
#define rho 1.0
#define mu 1.0
#define PE 1                   // Peclet number u*H/alpha (H = channel height LY)
#define U_REF 1.0

// ---- prescribed plug-flow velocity (liquid metal, Pr << 1) -----------------
#define U_CONST 1.0
#define V_CONST 0.0
...
#define CFL_EXPL 0.90          // safety factor on the explicit stability limit
#define DT_FLOOR 5.0e-8
#define TMAX 10.0
#define MAXSTEP 5000000
double WRITE_INTERVAL = 1e-1; //time interval between writes
double nextWriteTime = WRITE_INTERVAL;
#define DT_ACCURACY 1.0e-3
#define STEADY_TOL  1.0e-9
#define CSV_EVERY_STEP 0       // 1 -> CSV after every timestep (large output)
double io_s = 0.0;
...
// ---- temperature, T:  a*dT/dn + b*T = c ------------------------------------
#define AT_WEST   0.0
#define BT_WEST   1.0
#define CT_WEST   1.0          // inlet, T = 1

#define AT_SOUTH  0.0
#define BT_SOUTH  1.0
#define CT_SOUTH  0.0          // wall, T = 0

#define AT_EAST   1.0
#define BT_EAST   0.0
#define CT_EAST   0.0          // outlet, zero-gradient

#define AT_NORTH  0.0
#define BT_NORTH  1.0
#define CT_NORTH  0.0          // wall, T = 0

double BC_A_W, BC_B_W, BC_C_W;
double BC_A_S, BC_B_S, BC_C_S;
double BC_A_E, BC_B_E, BC_C_E;
double BC_A_N, BC_B_N, BC_C_N;

void VALIDATE_BC();

void SET_BC_COEFF()
{
    BC_A_W = AT_WEST; BC_B_W = BT_WEST; BC_C_W = CT_WEST;
    BC_A_S = AT_SOUTH; BC_B_S = BT_SOUTH; BC_C_S = CT_SOUTH;
    BC_A_E = AT_EAST; BC_B_E = BT_EAST; BC_C_E = CT_EAST;
    BC_A_N = AT_NORTH; BC_B_N = BT_NORTH; BC_C_N = CT_NORTH;
...
void VALIDATE_BC()
{
    ...
    const double den[4] = { BC_A_W + BC_B_W*dWb,
                            BC_A_S + BC_B_S*dSb,
                            BC_A_E + BC_B_E*dEb,
                            BC_A_N + BC_B_N*dNb };
    ...
    for(int w=0; w<4; w++)
    {
        ...
        {
            cout << "ERROR: " << wall[w] << " boundary condition on T"
...
void MAKE_OUTPUT_DIR();
void APPLYIC();
void APPLYBC(Field& PHI);
void FILL_CORNERS(Field& PHI);
double DT_EXPLICIT_LIMIT();
void SET_DELTAT();
void CALC_FLUXES();
...
void SOLVE_GS(const Field& AWx,  const Field& AEx,  const Field& ASx,
              const Field& ANx,  const Field& AWWx, const Field& AEEx,
              const Field& ASSx, const Field& ANNx, const Field& APx,
              const Field& PHI_OLD, Field& PHI, const char* tag);
void COMBINE();
void EXPLICIT_STEP();
...
int main(int argc, char* argv[])
{
    ...
    const char* usage = " [fou|sou|quick] [NI NJ] [imp|exp]";
    ...
    // time scheme: 4th argument, or 2nd when the grid size is omitted
    const char* tiArg = (argc > 4) ? argv[4] : ((argc == 3) ? argv[2] : NULL);

    if(tiArg)
    {
        TIME_SCHEME = PARSE_TIME_SCHEME(tiArg);
        if(TIME_SCHEME < 0)
        {
            cout << "usage: " <<  argv[0] << usage << endl;
            return -1;
    ...
    {
        int s = -1;
        cout << "TIME SCHEME: [0] IMPLICIT, [1] EXPLICIT -> ";
        if(!(cin >> s) || s < 0 || s > 1)
        {
            cout << endl << "no valid time scheme selected, defaulting to IMPLICIT" << endl;
            s = TI_IMPLICIT;
        }
        TIME_SCHEME = s;
    }
    ...
    cout << "TIME    : " << TIME_SCHEME_NAME() << endl;
    if(TIME_SCHEME == TI_IMPLICIT)
        cout << "SPLIT   : additive  ->  T^{n+1} = TA + TD - T^n" << endl;
    cout << "PHYSICS : liquid-metal plug flow   u = " << U_CONST << "   v = " << V_CONST
         << "   Pe = " << PE << "   alpha = " << scientific << setprecision(4)
         << alpha << endl;
    ...
    // velocity is prescribed, so the face fluxes are fixed for the whole run
    CALC_FLUXES();
    ...
    if(TIME_SCHEME == TI_EXPLICIT)
    {
        cout << "DT      : explicit limit = " << scientific << setprecision(3)
             << DT_EXPLICIT_LIMIT() << "   -> ~" << (long)ceil(TMAX/deltaT)
             << " steps to TMAX" << endl;

        if(ceil(TMAX/deltaT) > (double)MAXSTEP)
            cout << "WARNING : MAXSTEP = " << MAXSTEP << " will stop the run before TMAX" << endl;
    }

    WRITE_FILE_TRANSIENT();
    WRITE_FILE_TRANSIENT_VTK();
    WRITE_FILE_TRANSIENT_CSV();

    const int logEvery = (TIME_SCHEME == TI_EXPLICIT) ? 1000 : 20;

    // relative tolerance: accumulated round-off in simTime must not trigger a
    // final sliver step (dt ~ 1e-13 makes VOL/dt blow up and GS stall)
    while(TMAX - simTime > 1.0e-9*TMAX && TIMESTEP < MAXSTEP)
    {
        std::swap(T, T_old);
        ...
        if(TIME_SCHEME == TI_IMPLICIT)
        {
            ...
            SOLVE_GS(ADV_OPERATOR, T_old, TA, "ADV");
            SOLVE_GS(DIFF_OPERATOR, T_old, TD, "DIFF");
        ...
        const double dTdt = UPDATE_TRANSIENT();

        simTime += deltaT;
        TIMESTEP++;

        if(TIMESTEP % logEvery == 0)
        {
            cout << "STEP = " << setw(7) << TIMESTEP
                 << "   t = "  << fixed << setprecision(6) << simTime
                 << "   dt = " << scientific << setprecision(3) << deltaT
                 << "   ||dT/dt|| = " << setprecision(4) << dTdt;
            if(TIME_SCHEME == TI_IMPLICIT)
                cout << "   GS/step = " << TOTAL_GS_ITER/(long)TIMESTEP;
            cout << endl;
        }
    ...
    if(TIME_SCHEME == TI_IMPLICIT)
    {
        ...
        cout << "Total GS iterations = " << TOTAL_GS_ITER
             ...
             << " per timestep, 2 sub-solves per step)" << endl;
    }
    else
    {
        cout << "Total explicit steps = " << TIMESTEP << endl;
    }
...
void APPLYBC(Field& PHI)
{
    ...
    const double a1 = BC_A_W; const double b1 = BC_B_W; const double c1 = BC_C_W;
    const double a2 = BC_A_S; const double b2 = BC_B_S; const double c2 = BC_C_S;
    const double a3 = BC_A_E; const double b3 = BC_B_E; const double c3 = BC_C_E;
    const double a4 = BC_A_N; const double b4 = BC_B_N; const double c4 = BC_C_N;
...
void SET_DELTAT()
{
    ...
    const double cflrate = fabs(U_CONST)/DELX + fabs(V_CONST)/DELY;

    if(cflrate > 1.0e-12)
    {
        dt = min(dt, CFL_TARGET/cflrate);
    }

    if(TIME_SCHEME == TI_EXPLICIT)
    {
        dt = min(dt, DT_EXPLICIT_LIMIT());
    }
...
void CALC_FLUXES()
{
    // plug flow: uniform prescribed velocity, no coupling to the solution
...
double DT_EXPLICIT_LIMIT()
{
    ...
    // Cells next to a wall see the boundary node half a cell away, so their
    // diffusive coefficient sum is 3*alpha/d^2 rather than 2*alpha/d^2. Using
    // 3 keeps every cell's T_old[p] weight non-negative (bounded FOU solution).
    const double rate = wU*(fabs(U_CONST)/DELX + fabs(V_CONST)/DELY)
                      + 3.0*alpha/(DELX*DELX) + 3.0*alpha/(DELY*DELY);

    return CFL_EXPL/rate;
}
...
void CHECK_DIAG_DOMINANCE()
{
    ...
    // Dominance depends on dt, so this is checked every step and the running
    // minimum is reported at the end.
...
void SOLVE_GS(const Field& AWx,  const Field& AEx,  const Field& ASx, const Field& ANx,  const Field& AWWx, const Field& AEEx, const Field& ASSx, const Field& ANNx, const Field& APx, const Field& PHI_OLD, Field& PHI, const char* tag)
{
    PHI = PHI_OLD;
    APPLYBC(PHI);
...
void EXPLICIT_STEP()
{
    // Forward Euler on the same assembled operators as the implicit path:
    //   tr*(T - T_old) = sum_nb (C_nb + D_nb) T_old_nb - ((CP - tr) + (DP - tr)) T_old_P
    const int ni = NI;
    const double tr = VOL/deltaT;
    const double invtr = 1.0/tr;

    const double* RESTRICT to = T_old.data();
    double* RESTRICT t = T.data();
    ...
    for(int j=1; j<NCELLJ; j++)
    {
        for(int i=1; i<NCELLI; i++)
        {
            ...
            const double flux = (CW[p] + DW[p])*to[p-1]  + (CE[p] + DE[p])*to[p+1]
                              + (CS[p] + DS[p])*to[p-ni] + (CN[p] + DN[p])*to[p+ni]
                              + CWW[p]*to[PWW] + CEE[p]*to[PEE]
                              + CSS[p]*to[PSS] + CNN[p]*to[PNN]
                              - ((CP[p] - tr) + (DP[p] - tr))*to[p];
...
{
    char fname[512];
    snprintf(fname, sizeof(fname), OUTPUT_DIR "/temp_%05d.dat", TIMESTEP);
    ...
    out << "TITLE = \"2D Liquid-Metal Plug Flow - Temperature (cell-centred FVM)\"" << endl;
...
{
    char fname[512];
    snprintf(fname, sizeof(fname), OUTPUT_DIR "/temp_%05d.vtk", TIMESTEP);
    ...
    out << "2D Liquid-Metal Plug Flow - Temperature (cell-centred FVM), t = " << simTime << endl;
    out << "ASCII" << endl;
...
{
    char fname[512];
    snprintf(fname, sizeof(fname), OUTPUT_DIR "/temp_%05d.csv", TIMESTEP);
    ...
    out << "# t = " << simTime << ", step = " << TIMESTEP << endl;
    out << "x,y,T" << endl;

    for(int j=1;j<NCELLJ;j++)
        for(int i=1;i<NCELLI;i++)
            out << XC[i] << "," << YC[j] << "," << T[ID(j,i)] << "\n";
